  ...
  fetch("./data/photographers.json") // Get .json
    ...
    .then((data) => {
      ...
      const photographerToDisplay = data.photographers.find(
        // = objects containing this photographer id
      ...
      document.getElementById("photographerNameContact").textContent =
  ...
  const displayMediaDom = (medias) => {
    ...
    medias.forEach((media) => {
      if (media.hasOwnProperty("image")) {
        // Return boolean, true of false, if image...
    ...
    for (const [index, figure] of mediasCardsFigure.entries()) {
      ...
      const titleMediaClicked =
        figure.getElementsByTagName("h2")[0].textContent;
