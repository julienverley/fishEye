...
const photographerFactoryPage = (photographerObject) => {
  ...
  const { name, portrait, city, country, tagline } = photographerObject; // key/value name, portrait etc.
  const picture = `assets/photographersID/${portrait}`;

  const createPhotographerCardDOMPage = () => {
    // Build DOM
    ...
    span.innerHTML =
      '<button tabindex="4" class="contact-button" role="button" onclick="displayModal()">Contactez-moi</button>';
...
const createImageFactoryPage = (mediaObject) => {
  const { photographerId, title, image, likes } = mediaObject;
  const figure = document.createElement("figure");
  figure.classList.add("card");
  figure.setAttribute("tabindex", "5");

  const createImageFactoryPageDOM =
    // Gabarit or template literals
...
const createVideoFactoryPage = (mediaObject) => {
  const { photographerId, title, video, likes } = mediaObject;
  const figure = document.createElement("figure");
  figure.classList.add("card");
  figure.setAttribute("tabindex", "5");
