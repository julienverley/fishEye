...
const mediasFactoryPage = (mediaObject) => { // function attend un objet à traiter ; assigne les keys values aux 7 variables name, id etc.
    ...
    const medias = `assets/photographersMedias/${image}`; // à modifier pour assets/photographersMedias/82/... pex

    const createMediaCardDOMPage = () => { // PhotographerCardDOMPage créée dans le DOM
        const article = document.createElement( 'article' );
        const div = document.createElement( 'div' ); 
        const img = document.createElement( 'img' );
